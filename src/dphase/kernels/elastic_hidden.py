...
Computational cost
---------------------------------------------------------
Here, both scattering partners are dark-sector particles of comparable mass. The operator
therefore has to be a full 2 -> 2 redistribution, and the outgoing state depends on t as well
as s, so the quadrature needs BOTH CM angles, not just one:
...
Detailed balance
----------------
The event cache stores an unnormalized rate R per (k, l, i_chi, i_A) tuple
...
def _build_xA_elastic_cache_inner(
    ...
    mu_in_n, w_in_n   : GL nodes/weights for the lab-frame angle
    ...
    for k in range(Np):
        ...
        for l in range(Np):
            ...
            for mi in range(Nmu_in):
                # mu = cos(theta) between the incoming chi and A 3-momenta in the lab frame
                # mu_i fixes s, hence the CM energy s
                mu_i  = mu_in_n[mi]
                wmu_i = w_in_n[mi]
                ...
                if P_tot_sq <= 0.0:
                    ...
                    # with the lab, so the boost does nothing
                    gamma   = 1.0
                    cos_psi = 1.0
                    sin_psi = 0.0
                else:
                    ...
                    # incoming chi. This ties the CM emission angles to
                    ...
                    if cos_psi > 1.0:                # clamp
                        cos_psi = 1.0
                    elif cos_psi < -1.0:             # clamp
                        cos_psi = -1.0
                ...
                for mc in range(Nmu_cm):
                    # mc = index for cos(theta_CM) quadrature
                    # mu_c = cos(theta_CM), where theta_CM is the CM-frame scattering angle
                    mu_c = mu_cm_n[mc]
                    w_c  = w_cm_n[mc]

                    # |M|^2 inlined rather than calling model.xZD_xZD_m2, to keep
                    # this loop numba-compilable. For elastic scattering of equal
                    # CM momenta, t = -2 p_cm^2 (1 - cos).
                    ...
                    # phi (hence why it's computed one level up and
                    ...
                    for mp in range(Nphi):
                        ...
                        # the CM frame (see PDG kinematics review).
                        ...
                        # p3 is the lab-frame momentum of the outgoing chi particle
                        p3 = np.sqrt(p3_sq)
                        ...
                        # p4 is the lab-frame momentum of the outgoing A particle
                        p4 = np.sqrt(p4_sq)
                        ...
                        # Discretized loss rate, diagnostic only
                        ...
                        # Interpolation fraction for the chi particle between its bracketing bins
                        ...
                        # because we're depositing 4 sub-events corresponding to the four combinations of the bracketing bins
                        # the weights for these sub-events are given by the bilinear interpolation fractions
                        # contrast with xx -> AA where we deposit only 2 sub-events per scattering event
                        ...
                        # (i_chi_lo, i_A_lo)
                        # k, l = incoming chi and A momenta
                        ev_k_buf[n_ev]    = k
                        ev_l_buf[n_ev]    = l
                        # i_chi_lo, i_A_lo = indices for the outgoing chi and A particles in the momentum grid for this sub-event
                        ev_ichi_buf[n_ev] = i_chi_lo
                        ev_iA_buf[n_ev]   = i_A_lo
                        # R = rate weight for this sub-event
    ...
    # for each event we only care about: the incoming momenta (k, l), the outgoing momentum bin indices (i_chi, i_A), and the rate weight (R);
    # hence, we don't save any of the intermediate angular variables for each event.
...
def build_xA_elastic_gain_cache(
    ...
    dict with keys:
        ...
        K_loss        : (Np, Np) discretized loss rate, DIAGNOSTIC ONLY
    ...
    Navg = 6.0  # 2 chi spins x 3 A polarisations
    ...
    # interpolate into the same four bins. Collapsing them speeds up the apply step.

    # recall the factor of 4 comes from the four sub-events generated per scattering
    # event due to bilinear interpolation for both outgoing species
...
def _dedup_event_entries(k_idx, l_idx, i_chi, i_A, R, Np):
    ...
    Summing R is exact, not an approximation: apply step is linear in R for
    a fixed (k, l, i_chi, i_A), so merging entries changes nothing.

    This just shortens the event list by combining entries;
    it does not change the shape of any array.
    ...
    Returns
    -------
    The same arrays, deduplicated and with zero-rate entries dropped.
    """
    if k_idx.size == 0:
    ...
    # Sum the rates for events that share the same (k, l, i_chi, i_A) tuple.
...
def _apply_xA_delta_numba(k_idx, l_idx, i_chi, i_A, R, invW, f_chi, f_A, Np):
    ...
    Two properties follow: Delta vanishes whenever the
    ...
    Runs once per Newton iteration over the full deduplicated event list.
    """
    df_chi = np.zeros(Np)
    df_A   = np.zeros(Np)

    # loop over each EVENT, recalling that len(k_idx) == number of events
    for i in range(k_idx.shape[0]):
        # k, l = indices of the incoming momentum bins for chi and A, respectively
        k  = k_idx[i]
        l  = l_idx[i]

        # ic, iA = indices of the outgoing momentum bins for chi and A, respectively
        ic = i_chi[i]
        iA = i_A[i]

        # index into the current distributions to compute the delta for this event
...
def _xA_kernel_index(T, T_grid):
    ...
    Nearest-neighbor, matching every other lookup in `dphase.kernels`.
    """
