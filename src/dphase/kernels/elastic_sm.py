...
from dphase.model import gD_of_alpha, M2_chi_f_avg
...
def gamma_single_fermion(T, m_scatter, mf, Qf, Nc, gD, epsilon, mA):
    ...
    def integrand(omega):
        ...
        M2avg = M2_chi_f_avg(s, -k2_cm, m_scatter, mf, mA, gD, epsilon, Qf)
...
def build_xfxf_kernels(T_grid, state):
    ...
    After calling:
        ...
        state.gamma_grid_A    = np.array([g['A']   for g in gamma_grid])
    """
...
def gamma_from_grid(T, T_grid, gamma_arr):
    """
    interpolate precomputed gamma(T) via  log-linear interpolation.
    """
    log_T = np.log(np.asarray(T_grid, dtype=float))
    log_g = np.log(np.maximum(np.asarray(gamma_arr, dtype=float), 1e-300))
